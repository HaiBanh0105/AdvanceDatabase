                ...
                <?php foreach ($promotions as $promo): ?>
                    <div class="bg-white rounded-3xl border border-slate-100 shadow-sm hover:shadow-xl transition-all duration-300 overflow-hidden group">
                        ...
                        <div class="p-8 relative">
                            <div class="absolute -top-4 left-1/2 -translate-x-1/2 w-8 h-8 bg-white rounded-full border-t border-slate-100"></div>
                            <h4 class="text-lg font-bold text-slate-800 mb-3"><?= htmlspecialchars($promo['description']) ?></h4>
                            <div class="flex items-center justify-between mt-6 bg-slate-50 rounded-2xl p-2 border border-slate-100">
                                <span class="font-mono font-bold text-indigo-600 tracking-widest pl-4"><?= htmlspecialchars($promo['code']) ?></span>
                                <button onclick="navigator.clipboard.writeText('<?= htmlspecialchars($promo['code']) ?>'); showToast('Đã sao chép mã giảm giá!', 'success');" class="bg-indigo-600 text-white px-4 py-2 rounded-xl text-sm font-bold hover:bg-indigo-700 transition">Sao chép</button>
                            </div>
                        </div>
                    </div>
                <?php endforeach; ?>
