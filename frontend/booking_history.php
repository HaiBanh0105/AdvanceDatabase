...
// Lấy dữ liệu ảnh và tiện ích từ MongoDB
...
<body class="bg-slate-50">
                ...
                <div class="p-6 md:p-8 flex flex-col md:flex-row gap-6 items-center">
                    <div class="w-full md:w-48 h-32 rounded-2xl bg-slate-100 overflow-hidden shrink-0">
                        <?php
                                ...
                                if (!empty($b['type_id']) && isset($mongo_details[$b['type_id']]) && !empty($mongo_details[$b['type_id']]['base64'])) {
                                    $img = "data:" . $mongo_details[$b['type_id']]['mime'] . ";base64," . $mongo_details[$b['type_id']]['base64'];
                                }
                                ?>
                    ...
                    <div
                        ...
                        <div class="flex flex-col gap-2">
                            ...
                            <button onclick="confirmCancel('<?php echo $b['booking_id']; ?>', '<?php echo strtolower($b['status']); ?>')"
    ...
    function confirmCancel(id, status) {
        let confirmMsg = "Bạn có chắc chắn muốn hủy đơn đặt phòng này không?";
        
        if (status === 'confirmed') {
            confirmMsg += " Số tiền sẽ được hoàn về ví dựa trên chính sách của chúng tôi (Trừ phí hủy).";
        }
