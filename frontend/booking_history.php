...
$sql = "SELECT b.booking_id, b.booking_date, b.check_in, b.check_out, b.total_price, b.payment_method, b.status,
            (SELECT rt.name FROM Booking_detail bd JOIN Room r ON bd.room_id = r.room_id JOIN Room_types rt ON r.type_id = rt.type_id WHERE bd.booking_id = b.booking_id LIMIT 1) as room_name,
            (SELECT rt.image FROM Booking_detail bd JOIN Room r ON bd.room_id = r.room_id JOIN Room_types rt ON r.type_id = rt.type_id WHERE bd.booking_id = b.booking_id LIMIT 1) as room_image
        FROM Booking b 
        WHERE b.user_id = ? 
        ...
        <div class="space-y-6">
            ...
            <?php if (empty($bookings)): ?>
                <div class="bg-white rounded-[2rem] border border-slate-100 shadow-sm p-12 text-center">
                    ...
                    <a href="customer_index.php#rooms" class="inline-block bg-indigo-600 text-white px-8 py-3 rounded-2xl font-bold hover:bg-indigo-700 shadow-lg transition">Khám phá phòng ngay</a>
                </div>
            <?php else: ?>
                ...
                <div class="p-6 md:p-8 flex flex-col md:flex-row gap-6 items-center">
                    ...
                    <div class="flex-1 space-y-2 text-center md:text-left">
                        <div class="flex flex-wrap justify-center md:justify-start items-center gap-2 mb-1">
                            <span class="text-[10px] font-black uppercase tracking-widest <?php echo $is_cancelled ? 'text-slate-400' : 'text-indigo-600'; ?>">Mã đơn: #BK-<?php echo str_pad($b['booking_id'], 4, '0', STR_PAD_LEFT); ?></span>
                            
                            <?php if ($is_cancelled): ?>
                                <span class="px-2 py-0.5 rounded-full text-[9px] font-bold bg-slate-100 text-slate-500 uppercase">Đã hủy</span>
                            <?php elseif (strtolower($b['status']) === 'pending'): ?>
                                <span class="px-2 py-0.5 rounded-full text-[9px] font-bold bg-amber-100 text-amber-600 uppercase">Chờ xử lý</span>
                            <?php else: ?>
                                <span class="px-2 py-0.5 rounded-full text-[9px] font-bold bg-blue-100 text-blue-600 uppercase">Đã xác nhận</span>
                            <?php endif; ?>
                        </div>
                        <h3 class="text-xl font-bold <?php echo $is_cancelled ? 'text-slate-400' : 'text-slate-800'; ?> uppercase tracking-wide"><?php echo htmlspecialchars($b['room_name'] ?? 'Đa loại phòng'); ?></h3>
                        <div class="flex flex-wrap justify-center md:justify-start gap-x-6 gap-y-1 text-sm text-slate-500 font-medium">
                            <p><i class="fa-solid fa-calendar-day mr-2 text-indigo-400"></i>Nhận: <span class="text-slate-700"><?php echo date('d/m/Y', strtotime($b['check_in'])); ?></span></p>
                            <p><i class="fa-solid fa-calendar-check mr-2 text-indigo-400"></i>Trả: <span class="text-slate-700"><?php echo date('d/m/Y', strtotime($b['check_out'])); ?></span></p>
                        </div>
                    </div>
                    ...
                    <div class="w-full md:w-auto text-center md:text-right border-t md:border-t-0 md:border-l border-slate-100 pt-6 md:pt-0 md:pl-10 space-y-3">
                        <div>
                            <p class="text-[10px] font-bold <?php echo $is_cancelled ? 'text-slate-300' : 'text-slate-400'; ?> uppercase leading-none mb-1">Tổng thanh toán</p>
                            <p class="text-2xl font-black <?php echo $is_cancelled ? 'text-slate-300' : 'text-slate-900'; ?> uppercase"><?php echo number_format($b['total_price'], 0, ',', '.'); ?>đ</p>
                            <p class="text-[10px] font-bold text-emerald-500 uppercase mt-1"><?php echo htmlspecialchars($b['payment_method']); ?></p>
                        </div>
                        <div class="flex flex-col gap-2">
                            <?php if (!$is_cancelled): ?>
                                ...
                                <button onclick="confirmCancel('<?php echo $b['booking_id']; ?>')" class="text-slate-400 hover:text-red-600 text-[10px] font-bold uppercase transition">Hủy đặt phòng</button>
                            <?php else: ?>
